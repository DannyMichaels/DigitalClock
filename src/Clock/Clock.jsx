...
export default function Clock() {
  ...
  const defaultColor = "#77ff41";

  const [clockColor, setClockColor] = useState(() => {
    let localColor = localStorage.getItem("clockColor");
    return localColor ? localColor : defaultColor;
  });
    ...
    <ClockContainer clockColor={clockColor}>
      <div id="clock-skew">
        {timeArr.map((n, index) => (
          <div
            key={index}
            className={n < 10 ? "analog-digit-container" : "colon-container"}>
            {numbers[n].map((
              row,
              i // value of time array matches index of corresponding "number" matrix
            ) => (
              <div key={i * 1.23} className="row">
                {row.map((num, idx) => (
                  <div
                    key={idx * 2.94}
                    className={num === 1 ? "block on" : "block off"}></div>
                ))}
              </div>
            ))}
          </div>
      ...
      <ColorSelect clockColor={clockColor} setClockColor={setClockColor} />
    </ClockContainer>
  );
}
